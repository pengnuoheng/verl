...
When to use FSDP-Turbo
----------------------

Choose ``fsdp_turbo`` when you need:

* **Expert parallelism on Hugging Face models** without converting them to
  Megatron.  FSDP-Turbo shards expert weights across an EP group and dispatches
  tokens via an eager or fused dispatcher.
* **Context parallelism (CP)** through Ulysses-style sequence splitting, which
  allows long-sequence training that exceeds single-device memory.
* **CPU offloading** of parameters, optimizer states, and activations via
  ``CPUOffloadPolicy`` to fit large models on limited device memory.

For plain dense-model training without EP or CP, prefer the standard ``fsdp``
or ``fsdp2`` strategy—FSDP-Turbo adds no benefit in that case.
